...
public class UI { // Interface do Usuario
	...
	public static void imprimirPartida(PartidaDeXadrez partida, List<PecaDeXadrez> capturadas) {
		...
		System.out.println();
		System.out.println("Jogada: " + partida.getVez());
	...
	public static void imprimirPeca(PecaDeXadrez peca, boolean background) {
		if (background) {
	...
	private static void imprimirPecasCapturadas(List<PecaDeXadrez> capturada) {
		List<PecaDeXadrez> brancas = capturada.stream().filter(x -> x.getCor() == Cor.BRANCO).collect(Collectors.toList());
		List<PecaDeXadrez> pretas = capturada.stream().filter(x -> x.getCor() == Cor.PRETO).collect(Collectors.toList());
